...
class Collision():
    ...
    def perfect_collision(cls, gameimage1, gameimage2):
        ...
        offset_x = int(gameimage2.x - gameimage1.x)
        offset_y = int(gameimage2.y - gameimage1.y)

        if (isinstance(gameimage1,animation.Animation)):
            image1 = gameimage1.work_frames[gameimage1.curr_frame]
        else:
            image1 = gameimage1.image

        if (isinstance(gameimage1,animation.Animation)):
            image2 = gameimage1.work_frames[gameimage2.curr_frame]
        else:
            image2 = gameimage2.image

        #mask_1 = pygame.mask.from_surface(gameimage1.image)
        #mask_2 = pygame.mask.from_surface(gameimage2.image)

        mask_1 = pygame.mask.from_surface(image1)
        mask_2 = pygame.mask.from_surface(image2)
